...
class Server {
    ...
    async getViewerUrl(pdfUri) {
        ...
        const url = origUrl.toString() + (origUrl.toString().endsWith('/') ? '' : '/') + `viewer.html?file=${encodepath_1.PdfFilePathEncoder.encodePathWithPrefix(pdfUri)}`;
